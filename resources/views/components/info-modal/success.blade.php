...
<div {{ $attributes->merge(['class' => 'modal fade']) }} id="{{ $id }}" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-dialog-centered info-modal" role="document">
        <div class="modal-content info-modal-content">
            <i class="fa-regular fa-circle-check text-success info-modal-icon"></i>
            <div class="info-modal-title" id="successInfoModalTitle">{{ $title }}</div>
            <div class="info-modal-description" id="successInfoModalDescription">{{ $description }}</div>
            @if (isset($content))
                {{ $content }}
            @endif
